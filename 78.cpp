...
string SzO(string msg) {
int S[8] = { int('A'), int('L'), int('G'), int('O'), int('R'), int('Y'), int('T'), int('M') }; int length = msg.length();
int rest = length % 8;
...
string wynik = "";for (int i = 0; i < 8; i++) {
wynik += char(65 + S[i] % 26);
}
return wynik;}
...
ifstream fIn2;
ofstream fOut;
fIn1.open("C:\\podpisy.txt");fIn2.open("C:\\wiadomosci.txt");
fOut.open("C:\\epodpis_wynik.txt");
string napis1;
getline(fIn2, napis1);
fIn2.close();
    		fIn2.open("C:\\wiadomosci.txt");
   		cout << SzO(napis1)<<endl;
    cout << endl;
    			for (int i = 0; i < 11; i++) {
    int tab[8];
string napis;
